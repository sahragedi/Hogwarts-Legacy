...
public abstract class MagicItem implements Tradeable, MagicEffectRealization, MagicSource {
	...
	public MagicItem(String name, int usages, int price, int weight) {
		if(name == null || name.isEmpty()) throw new IllegalArgumentException("name cant be empty or null");
		if(usages < 0) throw new IllegalArgumentException("usages cant be neg");
		if(price <0) throw new IllegalArgumentException("price cant be neg");
	...
	public boolean provideMana(MagicLevel levelNeeded, int amount) {
		if(levelNeeded == null || amount < 0) throw new IllegalArgumentException("levelNeeded or amount cant be null");
		return true;
	}
	...
	public void takeDamagePercent(int percentage) {
		if(percentage < 0 || percentage > 100) throw new IllegalArgumentException("percentage out of range");
		double new_usages = usages*(1-percentage/100.00);
		usages = (int) new_usages;
	}
}
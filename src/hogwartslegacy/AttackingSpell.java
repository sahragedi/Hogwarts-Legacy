...
public class AttackingSpell extends Spell {
	...
	public AttackingSpell(String name,	int manaCost, MagicLevel levelNeeded, boolean type, boolean percentage, int amount) {
		...
		if(amount <0) throw new IllegalArgumentException("amount cant be neg");
		if(percentage==true && amount > 100) throw new IllegalArgumentException("amount out of range");
		this.type = type;
	...
	*/
	...
	public void doEffect(MagicEffectRealization target) {
		...
		if(target.isProtected(this)) {
			Set<AttackingSpell> spells = new HashSet<AttackingSpell>();
			spells.add(this);
	...
	public String additionalOutputString() {
		...
		return "; -%d%s %s".formatted(amount, p, t);
	}

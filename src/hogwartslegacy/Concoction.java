...
public class Concoction extends Potion {
	...
	public Concoction(String name,	int usages, int price, int weight, int health, int mana, List<Spell> spells) {
		...
		if(spells == null) throw new IllegalArgumentException("spells cant be null");
		if(health == 0 && mana == 0 && spells.isEmpty()) throw new IllegalArgumentException("health or mana cant be 0");
	...
	public String additionalOutputString() {
		String h_vorzeichen = health >= 0 ? "+" : "";
		String m_vorzeichen = mana >= 0 ? "+" : "";
		
		String h = health == 0 ? "" : "; %s%d HP".formatted(h_vorzeichen, health);
		String m = mana == 0 ? "" : "; %s%d MP".formatted(m_vorzeichen, mana);
		String s = spells.isEmpty() ? "" : "; cast %s".formatted(spells);
	...
	public void useOn(MagicEffectRealization target) {
		if(target == null) throw new IllegalArgumentException("target cant be null");
		if(!tryUsage()) return;

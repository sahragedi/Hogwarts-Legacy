...
public abstract class Spell {
	...
	public Spell(String name, int manaCost, MagicLevel levelNeeded) {
		if(name == null || name.isEmpty()) throw new IllegalArgumentException("name cant be empty");
		if(manaCost < 0) throw new IllegalArgumentException("manacost cant be neg");
	...
	public void cast(MagicSource source, MagicEffectRealization target) {
		...
		if(source == null || target == null) throw new IllegalArgumentException("source or target cant be null");
	boolean needed = source.provideMana(levelNeeded, manaCost);
	if(needed) 
		doEffect(target);
	}
	...
	public String toString() {
		String additional = additionalOutputString();
		
		return "[%s(%s): %d mana%s]".formatted(name, levelNeeded, manaCost, additional);	
		}
}
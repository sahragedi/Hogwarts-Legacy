...
public class Wizard implements MagicSource, Trader, MagicEffectRealization {
	...
	public Wizard(String name, MagicLevel level, int basicHP, int HP, int basicMP, int MP, int money,
		...
		if(name == null || name.isEmpty()) throw new IllegalArgumentException("name cant be null or empty");
		
		
		if(level == null) throw new IllegalArgumentException("level cant be null");
		
		if(basicHP < 0 || HP < 0) throw new IllegalArgumentException("hp or basichp cant be negative");
		
		if(basicMP < 0 || MP < 0) throw new IllegalArgumentException("basicmp or MP cant be negative");

        if (basicMP < level.toMana()) throw new IllegalArgumentException("mp cant be less than their magic level");

		if(money < 0) throw new IllegalArgumentException("money cant be negative");

		if(knownSpells == null) throw new IllegalArgumentException("known spells cant be null");

		if(protectedFrom == null) throw new IllegalArgumentException("protectedfrom cant be null");

		if(carryingCapacity < 0) throw new IllegalArgumentException("carrying capacity cant be negative");

		if(inventory == null) throw new IllegalArgumentException("inventory cant be null");

		int total = inventory.stream().mapToInt(Tradeable::getWeight).sum();
		if(total > carryingCapacity) throw new IllegalArgumentException("total weight cant exceed carrying capacity");
	...
	public boolean learn(Spell s) {
		if(s == null) throw new IllegalArgumentException("spell cant be null");
	...
	public boolean castSpell(Spell s, MagicEffectRealization target) {
		...
		if(s == null || target == null) throw new IllegalArgumentException("spell or target cant be null");
	...
	public boolean castRandomSpell(MagicEffectRealization target) {
		...
		Spell s = knownSpells.stream().findAny().get();
	...
	public boolean useItem(Tradeable item, MagicEffectRealization target) {
		if(item == null || target == null) throw new IllegalArgumentException("item or target cant be null");
	...
	public boolean useRandomItem(MagicEffectRealization target) {
		...
		Tradeable item = inventory.stream().findAny().get();
	...
	public String toString() {
		...
		return "[%s(%s): %d/%d %d/%d; %d %s; knows %s; carries %s]".formatted(name, level, HP, basicHP, MP, basicMP, money, knuts, knownSpells, inventory);
	}
	...
	public boolean provideMana(MagicLevel levelNeeded, int manaAmount) {
		if(manaAmount < 0) throw new IllegalArgumentException("amount neg");

		if(levelNeeded == null) throw new IllegalArgumentException("levelNeeded cant be null");
		if(isDead()) return false;
	...
	public boolean canAfford(int amount) {
		if(amount < 0) throw new IllegalArgumentException("amount neg");
	...
	public boolean hasCapacity(int weight) {
		if(weight < 0) throw new IllegalArgumentException("weight cant be neg");
	...
	public boolean addToInventory(Tradeable item) {
		...
		if(!inventory.contains(item) && hasCapacity(item.getWeight())) {
			inventory.add(item);
			return true;
		}
		return false;
	...
	public boolean steal(Trader thief) {
	    ...
	    if (thief == null) throw new IllegalArgumentException("Thief is null");
	        

	    if (!thief.canSteal() || inventory.isEmpty())
	        return false;

	    Tradeable stolenItem = inventory.stream().findAny().get();
	...
	public boolean loot(Trader looter) {
		if(looter == null) throw new IllegalArgumentException("looter cant be null");
	...
	public void takeDamage(int amount) {
		if(amount < 0) throw new IllegalArgumentException("cant be neg");
	...
	public void takeDamagePercent(int percentage) {
		if(percentage < 0 || percentage > 100) throw new IllegalArgumentException("perc out of range");
		int amount = (int) (basicHP*(percentage/100.0));
		if(HP < amount) HP = 0;
		else HP -= amount;
		//HP =  Math.max(HP - amount, 0);
	}
	...
	public void weakenMagicPercent(int percentage) {
		if(percentage < 0 || percentage > 100) throw new IllegalArgumentException("percentage neg");
	...
	public void setProtection(Set<AttackingSpell> attacks) {
		if(attacks == null) throw new IllegalArgumentException("attacks cant be null");

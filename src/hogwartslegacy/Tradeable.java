...
public interface Tradeable {
	...
	default boolean give(Trader giver, Trader taker) {
		if(giver == null || taker == null) throw new IllegalArgumentException("giver or taker cant be null");
		if(giver == taker) throw new IllegalArgumentException("giver and taker cant be same");
		
		int weight = getWeight();
		if(giver.possesses(this) && taker.hasCapacity(weight))
	...
	default boolean purchase(Trader seller, Trader buyer) {
		if(seller == null || buyer == null) throw new IllegalArgumentException("seller and buyer cant be null");
		if(seller == buyer) throw new IllegalArgumentException("seller and buyer cant be the same");
		int price = getPrice();
		int weight = getWeight();
